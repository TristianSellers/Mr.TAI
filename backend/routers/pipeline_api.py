...
from backend.services.ocr_scoresight import extract_scoreboard_via_scoresight
...
# Explicit emotion hint per (tone,gender)
EMOTION_BY_TONE_GENDER = {
    "neutral": {"male": "normal",  "female": "normal"},
    "radio":   {"male": "normal",  "female": "tonedown"},
    "hype":    {"male": "shout",   "female": "toneup"},
}
...
def voice_preview(
    ...
    # If not explicitly provided, use mapped voice and best-fit emotion
    if not voice_id:
...
def commentary_from_video(
    ...
    try:
        ocr = extract_scoreboard_via_scoresight()
...
def run_commentary_from_video(
    ...
    # NEW: accept explicit voice override from UI
    voice_id: Optional[str] = Form(None),
    emotion_preset: Optional[str] = Form(None),
):
    dest = _safe_save_upload(video, VIDEO_EXTS)
    ...
    try:
        ...
        return CommentaryRunResponse(
            ...
            meta={
                ...
                "provider": {"llm": type(llm).__name__, "tts": type(tts).__name__, "ocr": "scoresight"},
